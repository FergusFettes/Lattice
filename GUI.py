...
##====================Controlls on the left====================##
        ...
        hbWolf.addWidget(self.wolfScaleLabel)
        hbWolf.addWidget(self.wolfScale)
        hbWolf.addWidget(self.wolfCheck)
        self.textHolder.addLayout(hbWolf)

        # Boundary conditions
        self.bound = QCheckBox('Boundary')
        self.bound.setChecked(self.kwargs['BOUNDARY'])
        self.bound.stateChanged.connect(partial(self.Kwarger, 'BOUNDARY', self.bound.isChecked))
        self.boundPole = QSpinBox()
        self.boundPole.setRange(0, 1)
        self.boundPole.setValue(self.kwargs['POLARITY'])
        self.boundPole.setMaximumSize(100, 40)
        self.boundPole.valueChanged.connect(partial(self.Kwarger, 'POLARITY', self.boundPole.value))
        self.wolfLabel = QLabel('Wolfram Polarity')
        self.wolfPole = QSpinBox()
        self.wolfPole.setRange(0, 1)
        self.wolfPole.setValue(self.kwargs['WOLFPOLARITY'])
        self.wolfPole.setMaximumSize(100, 40)
        self.wolfPole.valueChanged.connect(partial(self.Kwarger, 'WOLFPOLARITY', self.wolfPole.value))
        hbPole = QHBoxLayout()
        hbPole.addWidget(self.bound)
        hbPole.addWidget(self.boundPole)
        hbPole.addWidget(self.wolfLabel)
        hbPole.addWidget(self.wolfPole)
        self.textHolder.addLayout(hbPole)
